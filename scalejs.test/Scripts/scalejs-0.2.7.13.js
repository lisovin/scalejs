    ...
    function createBuilder() {
        function builder(opts) {
            ...
            function build(context, cexpr) {
                ...
                if (expr.kind === '$bind') {
                    return opts.bind.call(context, expr.expr, function (bound) {
                ...
                if (expr.kind === 'return') {
                    return opts.returnValue.call(context, expr.expr);
                }

                if (expr.kind === '$return') {
                    return cexpr.length > 0
                        ? opts.combine(opts.returnValueFrom.call(context, expr.expr), build(context, cexpr))
                        : opts.returnValueFrom.call(context, expr.expr);
                }

                if (expr.kind === 'yield') {
                    return cexpr.length > 0
                        ? opts.combine(opts.yieldOne(expr.expr), build(context, cexpr))
                        : opts.yieldOne(expr.expr);
                }

                if (expr.kind === 'yieldMany') {
                    return cexpr.length > 0
                        ? opts.combine(opts.yieldMany(expr.expr), build(context, cexpr))
                        : opts.yieldMany(expr.expr);
                }
