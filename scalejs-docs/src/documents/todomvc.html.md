...
which is explained in more details in [Explanation and Overview](./overview.html#project).  
...
Functional modules are responsibile for implementing business functionality.
...
is scopped to the function passed into `define`. Optionally, you can define an array of arguments to be passed
...
of the MVVM functionality which is provided by [knockoutjs](linktoknockout) so it is a good idea to get familular
...
For example, our router is a parallel state and we will discuss routing later on.
The statechart is better understood in practice, but you can famuliarize yourself with the statechart from
the [statechart specification](linktostatechart). 

In order to preserve loose coupling, the statechart must allow modules to be agnostic to eachother, 
...
Styling main is simple and straight-forward. We used the [css](linktocss) that was provided by the [todomvc](linktodo)
...
The todo module is responsible for the functionality needed of the todo list.
To make a new module is extremely easy with scalejs due to the Visual Stuido Item templates
...
which will take whever value it is given and apply it to all the items' `completed` 
properties.
...
We also need to add a data-class to the input element so that we can toggle editting on
...
defined inline in the html, we also need to apply an `editting` class if the item is being 
...
is currently being editted, and `beginEdit` which is a function which will begin editting
on the item
...
This time, we made `title` an observable because when we are editting an item, presumably
...
Do this by clicking on the x, or editting an item and making it an empty string.
...
This can be tested by editting an item and instead of hitting the enter key, hit the escape key. 
